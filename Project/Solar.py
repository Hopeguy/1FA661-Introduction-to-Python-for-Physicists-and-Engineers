...
# win = tl.Screen()
# win.screensize(canvwidth=1280, canvheight=720, bg='black')
# win.title('Solar system')
...
planets_pos = [[57.9,0], [108.2,0], [149.6,0], [227.9,0], [778.6,0], [1433.5,0], [2872.5,0], [4495.1,0], [5906.4,0]]  # * 10**6 km, same y value as they are starting on a line "distance from sun"
...
planets_vel = [[47.4, 0], [35.0, 0], [29.8, 0], [24.1, 0], [13.1, 0], [9.7, 0], [6.8, 0],	[5.4, 0], [4.7, 0]] #km/s "orbital velocity"
# planets_radius = [4879,	12104,	12756,	6792,	142984,	120536,	51,118,	49528, 2370] #diameter in km
planets_radius = [10,	10,	10,	100,	100,	100,	100,	100,   100] #own values as they are very hard to see with the acctual radius as they differ so much between the planets, they are still assumed to be point like in calculations
...
for k, planet_2 in enumerate(planets_radius):
    planets_radius[k] = planet_2/2
    
Sun = Objects('Sun', [0,0], "yellow", 1.989*6, [0,0], 50)   #Correct radius is 696340/(10**6), but changed to be more visual appeling
...
Mars = Objects(planets_name[3], planets_pos[3], planets_color[3], planets_mass[3], planets_vel[3], planets_radius[3])
Jupiter = Objects(planets_name[4], planets_pos[4], planets_color[4], planets_mass[4], planets_vel[4], planets_radius[4])
Saturn = Objects(planets_name[5], planets_pos[5], planets_color[5], planets_mass[5], planets_vel[5], planets_radius[5])
Uranus = Objects(planets_name[6], planets_pos[6], planets_color[6], planets_mass[6], planets_vel[6], planets_radius[6])
Neptune = Objects(planets_name[7], planets_pos[7], planets_color[7], planets_mass[7], planets_vel[7], planets_radius[7])
Pluto = Objects(planets_name[8], planets_pos[8], planets_color[8], planets_mass[8], planets_vel[8], planets_radius[8])


#array with all plantes and sun

system = [Sun, Mercury, Venus, Earth, Mars, Jupiter, Saturn, Uranus, Neptune, Pluto]
#Turtle setup
